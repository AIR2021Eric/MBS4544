...
def canny(img):
    ...
    imgCanny = cv2.Canny(imgBlur, 50,130)
    return imgCanny
...
def region_of_interest(img):
    ...
    polygons = np.array([[(20,height),(150,height*2//3),(1100,height*2//3),(1250,height)]])
    # polygons = np.array([[(width*1//5, height), (width*1//3, height * 2 // 3), (width*2//3, height * 2 // 3), (width*4//5, height)]])
...
def average_slope_intercept(img, lines):
    ...
    if len(leftFit) > 0:
        # print(leftFit)
        # for i in leftFit:
        # arr = [(-0.6705882352941182, 891.6823529411764), (-0.6489028213166143, 870.1943573667709),
        #        (-0.6734006734006737, 877.6767676767678)]
        try:
            # find right most line in leftFit
            maxYIntercept = leftFit[0][1]
            id = 0
            for i in range(len(leftFit)):
                # print(leftFit[i][1])
                if leftFit[i][1] > maxYIntercept:
                    maxYIntercept = leftFit[i][1]
                    id = i
            # print(maxYIntercept)
            leftFitAvg = leftFit[id][0], maxYIntercept
            # print(leftFitAvg)
        except:
    ...
    if len(rightFit) > 0:
        try:
            # find left most line in rightFit
            minYIntercept = rightFit[0][1]
            id = 0
            for i in range(len(rightFit)):
                # print(leftFit[i][1])
                if rightFit[i][1] < minYIntercept:
                    minYIntercept = rightFit[i][1]
                    id = i
            # print(maxYIntercept)
            rightFitAvg = rightFit[id][0], minYIntercept
            # print(leftFitAvg)
        except:
            rightFitAvg = np.average(leftFit, axis=0)
...
video = cv2.VideoCapture("Resources/road_car_view.mp4")
prevLeft, prevRight = [], []
while True:
    ret, frame = video.read()
    Canny = canny(frame)
    croppedCanny = region_of_interest(Canny)
    lines = cv2.HoughLinesP(croppedCanny,2 ,np.pi/180, 100, np.array([]), minLineLength=5, maxLineGap=550)
    ...
    combinedImg = cv2.addWeighted(frame, 1.0, averageLinesImage, 0.8, 1)
